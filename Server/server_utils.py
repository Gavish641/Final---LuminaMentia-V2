...
class ScoresDB:
    ...
    def insert_score(self, username, game, score):
        ...
        if self.checkUserExists(username):
            last_encrypted_mean = json.loads(self.getMean(username))
            last_decrypted_mean = int(self.encryption.decrypt(eval(last_encrypted_mean[0]), eval(last_encrypted_mean[1]), eval(last_encrypted_mean[2]), eval(last_encrypted_mean[3])))
            new_mean = int((last_decrypted_mean*self.score_coefficient) + (score*(1-self.score_coefficient)))
            encrypted_new_mean = json.dumps(self.encryption.encrypt(str(new_mean)))
            encrypted_score = json.dumps(self.encryption.encrypt(str(score)))
            cursor.execute('''UPDATE scores SET lastScore=(?), mean=(?) WHERE username=(?) AND game=(?)''', (encrypted_score, encrypted_new_mean, username, game))
            print("rrrrrrrrrrrrrrrrrrrrrrrrrr")
        else:
            mean = score
            encrypted_score = self.encryption.encrypt(str(score))
            encrypted_new_mean = self.encryption.encrypt(str(mean))
            cursor.execute('''INSERT INTO scores (username, game, lastScore, mean) VALUES (?, ?, ?, ?)''', (username, game, json.dumps(encrypted_score), json.dumps(encrypted_new_mean)))
        conn.commit()
        cursor.close()
        conn.close()
...
class Encryption:
    def __init__(self, key=None):
        self.password_key = b"1234567890abcdef1234567890abcdef"
